...
# memory-save mode?
#  if the test sentence is short, and GPU mem space is large (>=11GB)
...
# There is no need to specify F0 mean and std because
#   mean/std has been written into trained_network.jsn by CURRENNT
F0EXT=''
...
# Here is the meanstd.bin I used (which may be different from
#  according the division of training and validation sets)
ACOUSMV=$PWD/DATATEMP/meanstd.bin

# trained model (either trained_network.jsn or epoch***.autoave )
MODELDIR=$PWD/MODELS/NSF
MODEL=${MODELDIR}/trained_network.jsn

# output directory
OUTDIR=${MODELDIR}/output_trained_network
...
if [ ! -e ${ACOUSMV} ]; then
    echo "Error: cannot find ${ACOUSMV}"
    exit
else
    echo "Use ${ACOUSMV}"
fi
