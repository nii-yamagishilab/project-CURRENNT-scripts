...
cmd="sh ./SCRIPTS/sub_03_waveform_mulaw_float.sh ${WAVFORMNORMDIR} ${MUWAVDIR} ${RAWWAVDIR} ${DATADIR}/scp/train.lst ${scriptsDir}"
cmdExt "${cmd}"
if [ -e ${DATADIR}/scp/val.lst ]; then
    cmd="sh ./SCRIPTS/sub_03_waveform_mulaw_float.sh ${WAVFORMNORMDIR} ${MUWAVDIR} ${RAWWAVDIR} ${DATADIR}/scp/val.lst ${scriptsDir}"
    cmdExt "${cmd}"
fi
check
...
cmd="sh ./SCRIPTS/sub_05_package_datanc.sh ${NCFLOATWAV}/DATA_TRAIN ${TIMEINDEXDIR} ${RAWWAVDIR} ${DATADIR}/scp/train.lst $PWD/CONFIGS/data_config.py ${scriptsDir}"
cmdExt "${cmd}"

if [ -e ${DATADIR}/scp/val.lst ]; then
    cmd="sh ./SCRIPTS/sub_05_package_datanc.sh ${NCFLOATWAV}/DATA_VAL ${TIMEINDEXDIR} ${RAWWAVDIR} ${DATADIR}/scp/val.lst $PWD/CONFIGS/data_config.py ${scriptsDir}"
    cmdExt "${cmd}"
fi
check
