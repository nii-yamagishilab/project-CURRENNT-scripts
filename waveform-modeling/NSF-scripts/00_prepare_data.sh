...
MUWAVDIR=${DATADIR}/wav16knorm_mulaw
RAWWAVDIR=${DATADIR}/wav16knorm_float

cmd="sh ./SCRIPTS/sub_03_waveform_mulaw_float.sh ${WAVFORMNORMDIR} ${MUWAVDIR} ${RAWWAVDIR} ${DATADIR}/scp/train.lst ${scriptsDir} >> ${LOG} 2>>${LOG}"
cmdExt "${cmd}"
if [ -e ${DATADIR}/scp/val.lst ]; then
    cmd="sh ./SCRIPTS/sub_03_waveform_mulaw_float.sh ${WAVFORMNORMDIR} ${MUWAVDIR} ${RAWWAVDIR} ${DATADIR}/scp/val.lst ${scriptsDir} >> ${LOG} 2>>${LOG}"
    cmdExt "${cmd}"
fi
check
...
NCFLOATWAV=${DATADIR}/ncData_floatWav
if [ ! -d ${NCFLOATWAV} ]; then
    mkdir ${NCFLOATWAV}
fi

cmd="sh ./SCRIPTS/sub_05_package_datanc.sh ${NCFLOATWAV}/DATA_TRAIN ${TIMEINDEXDIR} ${RAWWAVDIR} ${DATADIR}/scp/train.lst $PWD/CONFIGS/data_config.py ${scriptsDir} >> ${LOG} 2>>${LOG}"
cmdExt "${cmd}"

if [ -e ${DATADIR}/scp/val.lst ]; then
    cmd="sh ./SCRIPTS/sub_05_package_datanc.sh ${NCFLOATWAV}/DATA_VAL ${TIMEINDEXDIR} ${RAWWAVDIR} ${DATADIR}/scp/val.lst $PWD/CONFIGS/data_config.py ${scriptsDir} >> ${LOG} 2>>${LOG}"
    cmdExt "${cmd}"
fi
check
