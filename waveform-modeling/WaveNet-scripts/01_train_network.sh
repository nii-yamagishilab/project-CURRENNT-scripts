...
# No need to specify the extension of F0 for Wavenet
F0EXT=''
...
MODELDIR=$PWD/MODELS/wavenet001
...
DATANCDIR=$PWD/DATATEMP/ncData_muwav
...
if [ -e $PWD/CONFIGS/mdn.config ];then
    CURRENNTARG="${CURRENNTARG} --mdn_config $PWD/CONFIGS/mdn.config"
else
    echo "$PWD/CONFIGS/mdn.config generated by 00_prepare_data.sh is not found"
fi
