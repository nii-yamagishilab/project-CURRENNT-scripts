...
# memory-save mode? (AR Wavenet must be in memory-save mode)
MEMSAVEMODE=1
...
MODEL=$PWD/MODELS/wavenet001/trained_network.jsn

# output directory
OUTDIR=$PWD/MODELS/wavenet001/output_trained_network
...
if [ ! ${MEMSAVEMODE} -eq 1 ]; then
    showMessage "AR WaveNet only supports memory-save mode"
fi
...
CURRENNTARG="${CURRENNTARG} --ScheduleSampOpt 4 --ScheduleSampPara 0"
CURRENNTARG="${CURRENNTARG} --mdnSoftmaxGenMethod 2 --random_seed 12345231"
...
cmd="python ${scriptsDir}/wavScripts/genWav.py ${OUTDIR} ${MULAWBITS} ${WAVEFORMSR}"
cmdExt "${cmd}"
